...
mod tests {
    ...
    fn judge_circle() {
        ...
        let result: Vec<Vec<i32>> = vec![[0, 0, 0].to_vec(),[0, 0, 0].to_vec(), [0, 0, 0].to_vec()];
        assert_eq!(ImageSmoother::image_smoother(img),result);
        let img: Vec<Vec<i32>> = vec![[100,200,100].to_vec(),[200,50,200].to_vec(),[100,200,100].to_vec()];
        let result: Vec<Vec<i32>> = vec![[137,141,137].to_vec(),[141,138,141].to_vec(),[137,141,137].to_vec()];
        assert_eq!(ImageSmoother::image_smoother(img),result);
